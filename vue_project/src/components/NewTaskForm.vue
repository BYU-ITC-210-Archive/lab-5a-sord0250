<template>
    <div id="new-task-form">
      ...
      <v-form @submit.prevent="submitHandler(form)">
            ...
            <v-col cols="12" xs="12" sm="4">
              <v-menu v-model="menu" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
                <template v-slot:activator="{ on }">
                  ...
                  <v-text-field v-model="form.date" label="When should the task be done?" prepend-icon="event" @click="dateHandler(getCurrentDate)"></v-text-field>
  
                </template>
  
                <!-- TODO: Also bind this input to the Date property value of the form prop -->
                <v-date-picker v-model="form.date" @input="menu = false"  @click="dateHandler(formatDate)"></v-date-picker>
